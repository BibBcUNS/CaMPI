...
// -----------------------------------------------------------------------------
{
    ...
    if ( !isAACR2 && !isRDA ) {
        ...
        message += "Atención: este registro parece no haber sido creado usando AACR2 o RDA:";
...
// -----------------------------------------------------------------------------
{
    switch (tag) {
        ...
        case "041" :
            // Si hay más de un código de idioma en un mismo subcampo $a, creamos
            // subcampos adicionales ($a o $ h según el caso)
        ...
        case "240" :
            sf = sf.replace(/\^lEnglish/,"^lInglés");
        ...
        case "245" :
            ...
            // Uso de " ; " para preceder al subtítulo? (e.g. LCCN 73234889)
            ...
            if ( sf.search(/\^c/) == -1 ) {
                sf = sf.replace(/$/,"^c");  // Si no hay $c, colocamos uno (vacío)
                // ATENCION: no siempre es necesario un $c; quizás se pueda tomar la
                // presencia de un 100 o 700 como indicación de la probable necesidad de un $c.
                // ATENCION: revisar qué sucede cuando en realidad *no* debe ir un 245$c.
        ...
        case "300" :
            ...
            sf = sf.replace(/ of plates/," de láms.");
        ...
        case "500" :
            sf = sf.replace(/\^aTranslation of:? /,"^aTraducción de: ");
            sf = sf.replace(/\^aAt head of title: /,"^aCabecera de portada: ");
            sf = sf.replace(/\^aIncludes indexe?/,"^aIncluye índice");
            break;

        case "504" :
            sf = sf.replace(/\^aBibliography:/,"^aBibliografía:");
            //sf = sf.replace(/\^aIncludes bibliographies and index/,"^aIncluye referencias bibliográficas e índice");
            sf = sf.replace(/\^aIncludes bibliographies/,"^aIncluye referencias bibliográficas");
            sf = sf.replace(/\^aIncludes bibliographical references/,"^aIncluye referencias bibliográficas");
            sf = sf.replace(/ and indexe?/," e índice");
            break;
...
// -----------------------------------------------------------------------------
{
    ...
    nameToLatin1 = {
        ...
        '{ACUTE_ACCENT}C' : 'C',  // pérdida de información
        '{ACUTE_ACCENT}c' : 'c',  // pérdida de información
        ...
        '{HACEK}S' : 'S',  // pérdida de información (no anda bien poniendo '¦')
        '{HACEK}s' : 's',  // pérdida de información (no anda bien poniendo '¨')
        '{HACEK}C' : 'C',  // pérdida de información
        '{HACEK}c' : 'c',  // pérdida de información
