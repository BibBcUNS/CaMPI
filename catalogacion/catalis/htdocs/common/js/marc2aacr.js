...
// -----------------------------------------------------------------------------
function marc2aacr(materialType,f001,f005,f008,marcDatafields,ejemplares,imageFileType)
...
// -----------------------------------------------------------------------------
{
    ...
    // Reemplazo de caracteres < y > (FG, 2008-09-17)
    for (var i = 0, len = marcDatafields.length; i < len; i++) {
        marcDatafields[i] = marcDatafields[i].replace('<', '&lt;').replace('>', '&gt;');
    }
    ...
    //
    // Tomamos solo la 1ra ocurrencia que tenga I1 = 0. (arreglado FG 2008-10-08)
    // ----------------------------------------------------------
        if ( fields_byTag["362"] ) {
                for (var i = 0; i < fields_byTag["362"].length; i++) {
                        if ( "0" == fields_byTag["362"][i].substr(0,1) ) {
                                description += " " + LONG_DASH + " ";
                               description += "<span " + fieldHighlight("362") + ">";
                               description += fields_byTag["362"][i].substr(4).replace(/\^\w/g," ").replace(/-$/,"-&nbsp;&nbsp;&nbsp;&nbsp;").replace(/^-/,"&nbsp;&nbsp;&nbsp;&nbsp;-");
                               description += "</span>";
                               description += ".";
                               break;
                        }
    ...
    if ( fields_byTag["264"] ) {   // TO-DO: Repetible
        description += " " + LONG_DASH + " ";
    ...
    if ( fields_byTag["773"] && fields_byTag["773"][0].substr(0,1) == "0" ) {
        ...
        for (var i=0; i < subfieldArray.length; i++) {
            ...
            if ( code.search(/[hmnruwyz7]/) == -1 ) {
                ...
                description += subfieldArray[i].substr(1).replace(/@@@./g,' ') + " ";
            }
        }
    }
    ...
    if ( fields_byTag["362"] ) {
        for (var i = 0; i < fields_byTag["362"].length; i++) {
            if ( "1" == fields_byTag["362"][i].substr(0,1) ) {
    ...
    for (var i=0; i < fields_inOrder.length; i++)  {
        if ( fields_inOrder[i].tag.substr(0,1) == "5" ) {
            ...
            switch (tag) {
                ...
                case "510" :   // Citation/references note
                    ...
                    switch ( ind1 ) {
                        ...
                        case "3" :
                        case "4" :
                            displayConstant = ( "bibima" == g_activeDatabase.name ) ? "Reseña: " : "Referencias: "; // References
                            break;
                    }
                    break;
            ...
            if ( "510" == tag && "bibima" == g_activeDatabase.name && extractSubfield(fields_inOrder[i].value,"a").search(/^(Math\. Rev\.|MR),$/) != -1 ) {
                description += "MR, ";
                ...
                for ( var j=0; j < reviewNumbers.length; j++ ) {
                    ...
                    description += "<a target='_blank' href='" + mrURL + "' title='Ver registro en MathSciNet' _onclick='auxWin(this.href); return false'>";
            ...
            // TODO: Agregar enlace cuando el 510 contiene un subcampo $u
        }
    }
    ...
    for (var i=0; i < fields_inOrder.length; i++) {
        if ( fields_inOrder[i].tag.substr(0,3).search(/76.|77[^3]|78./) != -1 && "0" == fields_inOrder[i].value.substr(0,1) ) {
            ...
            description += "<span class='displayConstant'>" + displayConstant + "</span>";
            description += fields_inOrder[i].value.replace(/\^w[^\^]+/g,"").replace(/\^i([^\^]+)/,"^i<span class='displayConstant'>$1</span>").substr(4).replace(/\^x/g, "^xISSN ").replace(/\^z/g,",^zISBN ").replace(/\^\w/g," ");
            description += ".";
        }
    }
    ...
    // FIXME - Hay error si el 022 tiene mas ocurrencias que el 222 (FG, 2008-12-17).
    // ---------------------------------------------------------
    if ( fields_byTag["022"] ) {
        ...
        for (var i=0; i < fields_byTag["022"].length; i++) {
            ...
            description += "ISSN ";
            //description += fields_byTag["022"][i].substr(4).replace(/\^z[^\^]+/g, "").replace(/\^\w/g, " ");
            description += extractSubfield(fields_byTag["022"][i],"a");  // FG, 2008-10-15
            if ( fields_byTag["222"] ) {
    ...
    if ( fields_byTag["010"] ) {
        // tratamiento de '#' corregido (FG 2008-09-29)
        var LCCNnumber = extractSubfield(fields_byTag["010"][0],"a");
        //var LCCNlink = "<a target='_blank' href='http://catalog.loc.gov/cgi-bin/Pwebrecon.cgi?v4=1&DB=local&CMD=010a+" + LCCNnumber.replace(/#/g,"+") + "&CNT=10+records+per+page'>LCCN " + LCCNnumber.replace(/#/g,"") + "</a>";
        //var LCCNlink = "<a target='_blank' href='https://catalog.loc.gov/vwebv/search?searchCode=STNO&searchArg=" + LCCNnumber.replace(/#/g,"") +  "&searchType=1'>LCCN " + LCCNnumber.replace(/#/g,"") + "</a>";
        var LCCNlink = "<a target='_blank' href='https://lccn.loc.gov/" + LCCNnumber.replace(/#/g,"") + "'>LCCN " + LCCNnumber.replace(/#/g,"") + "</a>";
    ...
    // Imagen (FG, 2017/09/01)
    var database = window.top.g_activeDatabase.name;
    var imageUrl = window.top.HTDOCS + "img/";
    if (imageFileType) {
    ...
    if ( DISPLAY_CALL_NUMBER ) {
        var aacrTable = "";
        aacrTable += "<table id='aacrTable' cellpadding='0' cellspacing='0' border='0'>";
        aacrTable += "<tr>";
        aacrTable += "<td id='documentLocationCell'>" + documentImage + documentLocation + "</td>";
