...
// leaderData: sólo los 8 elementos que nos interesan (05,06,07,08,09,17,18,19)
// -----------------------------------------------------------------------------
{
	var form = document.getElementById("marcEditForm");
	...
	if (ie) {
		form.L_06.title = xmlFixedField.selectNodes(myPath)[0].getAttribute("name");
	} else if (moz) {
		form.L_06.title = xmlFixedField.evaluate(myPath,xmlFixedField,null,9,null).singleNodeValue.getAttribute("name");
	}
	//form.L_06.style.cursor = "help";
	
	var myPath = "/" + "/dataElement[@pos='L_07']/option[@code='" + form.L_07.value + "']";
	if (ie) {
		form.L_07.title = xmlFixedField.selectNodes(myPath)[0].getAttribute("name");
	} else if (moz) {
		form.L_07.title = xmlFixedField.evaluate(myPath,xmlFixedField,null,9,null).singleNodeValue.getAttribute("name");
	}
...
// -----------------------------------------------------------------------------
{
	...
	tooltip = form.f008_06.value + " : " + xmlFixedField.selectNodes(myPath)[0].getAttribute("name");
	document.getElementById("TD_f008_06").title = tooltip;

	myPath = "/" + "/country[@code='" + form.f008_15_17.value + "']";
	try {
		tooltip = form.f008_15_17.value + " : " + xmlCountryCodes.selectNodes(myPath)[0].getAttribute("name");
	...
	try {
		tooltip = form.f008_35_37.value + " : " + xmlLanguageCodes.selectNodes(myPath)[0].getAttribute("name");
