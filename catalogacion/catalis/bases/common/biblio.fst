0000 0 /* ================================================= */
0000 0 /*  FST para bases bibliograficas MARC 21            */
0000 0 /*                                                   */
0000 0 /*  Compartida por el OPAC y Catalis.                */
0000 0 /*                                                   */
0000 0 /*  Está en permanente desarrollo.                   */
0000 0 /*                                                   */
0000 0 /*  (c) 2003 Fernando J. Gomez - CONICET - INMABB    */
0000 0 /* ================================================= */


0000 0 /* ================================================= */
0000 0 /*             Nro. de control                       */
0000 0 /* ================================================= */
9001 0 '-NC=',v001

0000 0 /* ================================================= */
0000 0 /*          Tipo de material (default: 'a')          */
0000 0 /* ================================================= */
...
0000 0 /*                     Fecha                         */
0000 0 /* ================================================= */
9008 0 '-F=',v008*07.4  /* date1 */
9008 0 if v008*11.4 <> '####' then '-F=',v008*11.4 fi /* date2 */
...
0000 0 /* ================================================= */
0000 0 /*             Otra informacion codificada           */
0000 0 /* ================================================= */
9008 0 if v008*30.1='1' then '-FEST' fi
9008 0 if '0eis' : v008*33.1 then '-FICTION=NO' else if '1cdfhjmp' : v008*33.1 then '-FICTION=YES' fi,fi
9008 0 if 'abcd' : v008*34.1 then '-BIOGR=YES' fi
9008 0 if s(v008*24.4) : 'd' or s(v008*24.4) : 'e' then '-REFERENCE' fi /* diccionarios, enciclopedias */
9050 0 /* if p(v050) then ( '-LCC=', if '0123456789' : v050^a*1.1 then v050^a.1 else v050^a.2 fi/ ), fi */
9995 0 /* (|-BIB=|v995/) */

0000 0 /* Libros electronicos */
9008 0 if v008*23.1 = 's' and s(v906,v907) = 'am' then '-EBOOK' fi


0000 0 /* ================================================= */
0000 0 /*               Lugar de publicacion                */
0000 0 /* ================================================= */
...
9260 4 /* ref(['COUNTRY']l(['COUNTRY']replace(replace(v008*15.3,'#',''),'_','')),v2) -- GENERA CLAVES FANTASMAS */
...
0000 0 /* Titulos en la descripcion */
0000 0 /* ========================= */
...
0000 0 /* Titulos en puntos de acceso */
0000 0 /* =========================== */
...
0000 0 /* ================================================= */
0000 0 /*     Titulos completos (para links, tecnica 0)     */
0000 0 /* ================================================= */
...
9200 0 if p(v730)   then proc('d1000',  'a1000¦',replace(v730*3,'^','¦a1000¦'),'¦'),                ,(,                            if 'adfgkmnoprst': v1000.1 then '~'v1000*1/ fi    ,), fi,
...
0000 0 /* ================================================= */
0000 0 /*          Series (para links, tecnica 0)           */
0000 0 /* ================================================= */
0000 0 /* ATENCION: revisar subcampos (quitar $6,$8), repetibilidad, etc. */
9400 0 if p(v440) then proc('d1000',(,'a1000¦',replace(v440*3,'^','¦a1000¦'),'¦','a1000¦##¦',), ), ,(,if v1000 = '##' then / else if 'a': v1000.1 then '~'v1000*1, fi,fi ,), fi,
...
9084 0 if s(mpu,v084^2) : 'MSC' then ('-MSC=',v084^a/) fi
...
0000 0 /* ================================================= */
0000 0 /* La mención de responsabilidad, etc.               */   /* FG, 2012-08-24 */
0000 0 /* ================================================= */
9000 4 v245^c
...
0000 0 /* ================================================= */
9859 0 if p(v859) then ( '-INV=',v859^p/ ) fi
9859 0 if p(v859) then ( '-ST=', if s(v859^k,v859^h,v859^i) = '' or size(v859^i) < 1 then '--' else if v859^k<>'' then v859^k,x1, fi, v859^h, if v859^i<>'' then x1,v859^i, fi, fi / ) fi
9859 0 if p(v859) then ( if v859^o <> '' then '-PAT=',v859^o/, fi ), fi /* Datos de patrimonio (UNS) */
...
9980 0 if v980^s : 'OK' then '-MSC=OK' else if v980^s : 'DUDA' then '-MSC=DUDA' fi,fi,
9980 0 if p(v980^o) or (p(v980) and not v980 : '^') then '-ANOTACION-OTRA' fi
9991 0 '-CREADO_POR=',v991

9993 0 if v993 = '1' then '-ETIQ=si' else '-ETIQ=no' fi /* Para etiquetas de bibima. FG, 2013-08-20 */

0000 0 /* La siguiente linea permite resolver el error de la 2da grabacion en Catalis */
0000 0 proc('d1000')