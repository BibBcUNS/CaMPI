...
<body>
	...
	<tr><td align="right">Seleccione la BD</td><td align="left" width="55%">
		...
		for($i=0;$i<count($bases)-1;$i++){
			...
			if ($bases[$i] == "ucod-marc" || $bases[$i] == "carpc" || $bases[$i] == "bibadm" || $bases[$i] == "bibeco")
	...
	<tr><td>
		...
		for($i=0;$i<count($bases)-1;$i++){
		    // Esto es una restricción QUE HAY QUE BORRAR
			if ($bases[$i] == "ucod-marc"  || $bases[$i] == "bibadm" || $bases[$i] == "bibeco")
			echo "<option value=$bases[$i]>$bases[$i]</option>";}
		?>
		</select>
	...
	<tr><td>
		...
		for($i=0;$i<count($bases)-1;$i++){
		    // Esto es una restricción QUE HAY QUE BORRAR
			if ($bases[$i] == "eunm" || $bases[$i] == "ead" || $bases[$i] == "eeo"  )
			echo "<option value=$bases[$i]>$bases[$i]</option>";}
		?>
		</select>
